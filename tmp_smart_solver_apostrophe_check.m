function out = tmp_smart_solver_apostrophe_check(varargin)
% Auto-generated by EIDAT Smart Solver MATLAB export.
% Prediction usage: y = tmp_smart_solver_apostrophe_check(Input1)
% Metadata usage: meta = tmp_smart_solver_apostrophe_check('metadata')
% Inputs: Input1 (Input1) [u]
% Output: y is predicted Thruster's Output; scalar and array inputs are evaluated element-wise.
...
if false
    ...
    meta = tmp_smart_solver_apostrophe_check('metadata');
    Input1 = 1;
    pred = tmp_smart_solver_apostrophe_check(Input1);
    actual_mean = 3;
    fprintf('Equation: %s\n', meta.equation_text);
    fprintf('Predicted Thruster''s Output: %.12g\n', pred);
...
if nargin ~= 1
    error('tmp_smart_solver_apostrophe_check:Usage', 'Usage: y = tmp_smart_solver_apostrophe_check(Input1); meta = tmp_smart_solver_apostrophe_check(''metadata'');');
end
Input1 = varargin{1};
out = ((2.*Input1) + 1);
end
...
meta = struct();
meta.output_target = 'Thruster''s Output';
meta.output_units = 'u';
meta.input_targets = {'Input1'};
meta.input_units = {'u'};
meta.input_roles = {'Primary'};
meta.argument_names = {'Input1'};
meta.equation_text = 'y = 2*x + 1';
meta.normalization_text = '';
meta.fit_family = 'polynomial';
meta.fit_family_label = 'Polynomial';
meta.solver_branch = 'polynomial';
meta.selection_reason = 'Simple line.';
meta.uses_control_period = false;
meta.uses_staged_mediator = false;
meta.selected_control_period = [];
meta.rmse = 0;
meta.residual_threshold = 0;
meta.in_fit_percent = 0;
meta.in_fit_count = 0;
meta.fell_out_count = 0;
meta.fell_out_percent = 0;
meta.sample_count = 0;
meta.warning_text = '';
meta.run_count = 0;
meta.serial_count = 0;
